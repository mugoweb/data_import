...
class ImportOperator
{
	...
	public function run()
	{
		...
		while( $this->source_handler->getNextRow() && !$force_exit )
		{
			...
			$this->cli->output( 'Handling object ('.$this->cli->stylize( 'emphasize', $remoteID ).') [', false );
	 ...
	 * 
	 * @param string $targetContentClass
	 * @param string $targetLanguage
	 * @return boolean
	 */
	 ...
	 */
	protected function save_eZ_nodes( &$force_exit )
	{
		$new_dom_nodes = $this->source_handler->getDomNodes();
		
		foreach( $new_dom_nodes as $dom_node )
		{
		...
		if( $parentNodeRemoteID )
		{
			...
			if( $parentNode )
			{
				...
				$nodeInfo = array(
						...
						#'priority'              => $dom_node->getAttribute( 'priority' ),     // doesn't work
				...
				{
					$this->cli->output( 'adding location, ', false );
				...
				{
					$this->cli->output( $this->cli->stylize( 'red', 'Cannot create a node-assignment that already exists. ' ), false );
				}
			}
			else
			{
